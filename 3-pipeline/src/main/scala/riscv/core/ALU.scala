...
class ALU extends Module {
  ...
  switch(io.func) {
    ...
    is(ALUFunctions.sra) {
      // Implement arithmetic right shift (Shift Right Arithmetic)
      // Hint: Need to preserve sign bit:
    ...
    is(ALUFunctions.slt) {
      ...
      // Convert both operands to signed type then compare
    ...
    is(ALUFunctions.sltu) {
      ...
      // Directly compare unsigned values
