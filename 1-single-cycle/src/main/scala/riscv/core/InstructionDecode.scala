...
class InstructionDecode extends Module {
  ...
  // TOD: Determine when to write back from Memory
  when(isLoad) {
    wbSource := RegWriteSource.Memory
  }
  // TOD: Determine when to write back PC+4
  ...
  // TOD: Determine when to use PC as first operand
  ...
  // TOD: Determine when to use immediate as second operand
  ...
  //
  // TOD: Complete S-type immediate extension
  val immS = Cat(
  ...
  //
  // TOD: Complete B-type immediate extension
  val immB = Cat(
  ...
  //
  // TOD: Complete J-type immediate extension
  val immJ = Cat(
