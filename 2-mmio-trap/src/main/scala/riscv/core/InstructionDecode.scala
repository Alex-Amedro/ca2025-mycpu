...
class InstructionDecode extends Module {
  ...
  // Determine when to write back from Memory
  when(isLoad) {
    wbSource := RegWriteSource.Memory
  }
  // Determine when to write back from CSR
  .elsewhen(isCsr) {
    wbSource := RegWriteSource.CSR
  }
  // Determine when to write back PC+4
  ...
  // Determine when to use PC as first operand
  ...
  // Determine when to use immediate as second operand
